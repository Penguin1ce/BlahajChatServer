...
// ListMessageByConvID 按 ID 倒序拉取会话历史。beforeID 为 0 时表示从最新一条开始。
func ListMessageByConvID(ctx context.Context, convID string, beforeID uint64, limit int) ([]model.Message, error) {
	if limit <= 0 {
		limit = 20
	}
	if limit > 100 {
...
func GetMessageByID(ctx context.Context, msgID string) (*model.Message, error) {
